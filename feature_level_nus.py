...
save_path = '../Datasets/NUS_FLICK/perturb_eps_{}_gamma_{}'.format(epsilon, gamma)
if (os.path.isdir(save_path) == False):
    os.makedirs(save_path) 

feat_matrix = np.load('../Datasets/NUS_FLICK/nus_resnet50_plain_feat.npy')
mask_matrix = np.load('../Datasets/NUS_FLICK/nus_resnet50_mask_feat.npy')
shap_score = np.load('../Datasets/NUS_FLICK/nus_resnet50_shap_score.npy')
...
for batch in tqdm(range(num_batch)):
    ...
    overall_score = gamma*shap_score[batch*batch_size:(batch+1)*batch_size,:] + (1-gamma)*sens_score[batch*batch_size:(batch+1)*batch_size,:]
