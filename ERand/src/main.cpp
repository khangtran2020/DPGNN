...
// redistributed and modified under the terms of the GNU General Public License. Please see the
// detailed copyright claims in their programs and give credits to original authors of HRG if you use
// these programs.
//
...
struct ioparameters {
    ...
    int n_priv;         // number of private vertices
    long long m;                // number of edges in input graph
    int num_priv;
    int num_pub;
    ...
    if (parseCommandLine(argc, argv)) {
        ...
        if (ioparm.flag_f) {
            ...
            if (ioparm.epsilon_hrg) {                     //compute parameter T to adjust the underlying distribution
                double N = ceil(ioparm.n_priv / 2.0) * floor(ioparm.n_priv / 2.0);
                double p = 1.0 / N;
            ...
            if (!(MCMCEquilibrium_Sample(num_samples, ioparm.T,
                                         flag_control))) { return 0; }    // sample likelihoods for missing connections
            ...
            if (!(d->importDendrogramStructure(ioparm.f_dg))) {
            ...
            while (i < num_of_graph) {
                ...
                d->makeNoisyRandomGraph(randomG, ioparm.epsilon_edge,
                                        flag_control); //use Erdős–Rényi random graph model if flag_control is true
...
bool parseCommandLine(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        cout << "  -psilonHRG <real number>    Input privacy budget for HRG\n";
        ...
        while (argct < argc) {
            ...
            if (temp == "-make" and !ioparm.flag_f) {
                ...
                pos = temp.find(ext, 0);
                if (pos == string::npos) {
                    cout << " Error: Input file must claim to be .hrg format.\n";
                    return safeExit;
                }
...
//                for (int i=0; i < temp.size(); i++) { if (temp[i] == '.') { pos = i; } }
            ...
            } else if (temp == "-pub_round") {
                argct++;
                ioparm.num_pub = stoi(argv[argct]);
            } else if (temp == "-priv_round") {
                argct++;
                ioparm.num_priv = stoi(argv[argct]);
            } else if (temp == "-compact") {
                ioparm.flag_compact = true;
            } else if (temp == "-epsilonHRG") {
                argct++;
                ioparm.epsilon_hrg = strtod(argv[argct], NULL);
            } else if (temp == "-epsilonE") {
                argct++;
                ioparm.epsilon_edge = strtod(argv[argct], NULL);
            } else if (temp == "-eq") {
                argct++;
                ioparm.thresh_eq = int(strtod(argv[argct], NULL));
            } else if (temp == "-stop") {
                argct++;
                ioparm.thresh_stop = int(strtod(argv[argct], NULL));
...
//        ext = "/";
        ioparm.out_dir = timestampDir + ext + ioparm.s_scratch;
...
bool readPairsFile() {
    ...
    while (fscan1 >> s >> f >> type) {                    // read friendship pair (s,f)
        if (s != f) {
    ...
    while (fin >> s >> f >> type) {
        m++;
        if (s != f) {
            item = namesLUT.findItem(s);
            a = item->value;
            item = namesLUT.findItem(f);
            b = item->value;
            if (!(d->g->doesLinkExist(a, b, type))) {
                if (!(d->g->addLink(a, b, type))) {
                    cout << "Error: (" << s << " " << f << ")" << endl;
                }
                else if (d->g->getName(a) == "") { d->g->setName(a, num2str(s));}
            }
            if (!(d->g->doesLinkExist(b, a, type))) {
                if (!(d->g->addLink(b, a, type))) {
                    cout << "Error: (" << s << " " << f << ")" << endl;
                }
                else if (d->g->getName(b) == "") { d->g->setName(b, num2str(f));}
            }
        }
        if (t2 - t1 > ioparm.timer) {                // check timer; if necessarsy, display
            cout << "   edges: [" << m << "]" << endl;
    ...
    ioparm.n_priv = d->g->numNodePrivate();
    cout << "vertices: [" << ioparm.n << "]" << endl;
    cout << "Private vertices: [" << ioparm.n_priv << "]\n";
...
bool MCMCEquilibrium_Find(double T) {
    ...
    if (ioparm.thresh_eq) {
        thresh_eq = max(ioparm.thresh_eq, 50);
    } else {
        thresh_eq = 50;
    }
    if (ioparm.thresh_stop) {
        thresh_stop = max(ioparm.thresh_stop, 50);
    } else {
        thresh_stop = 50;
    }
    ...
    while (!flag_eq) {
        ...
        for (int i = 0; i < interval; i++) {
            if (move < ioparm.num_pub) {
                if (!(d->monteCarloMove(dL, flag_taken, T, true))) {    // Make a single MCMC move
                    return false;
                }
                move++;
            }
            else {
                if (move < ioparm.num_pub + ioparm.num_priv - 1) {
                    if (!(d->monteCarloMove(dL, flag_taken, T, false))) {    // Make a single MCMC move
                        return false;
                    }
                    move++;
                }
