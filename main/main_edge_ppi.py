...
dataset_train = PPIDataset(eps_feat=epsilon_feat, eps_edge=epsilon_edge, mode='train', p_rate=private_edge_rate, exp_mode='edge')
dataset_val = PPIDataset(eps_feat=epsilon_feat, eps_edge=epsilon_edge, mode='valid', p_rate=private_edge_rate, exp_mode='edge')
dataset_test = PPIDataset(eps_feat=epsilon_feat, eps_edge=epsilon_edge, mode='test', p_rate=private_edge_rate, exp_mode='edge')
...
for run in range(num_run):
    ...
    name_model_to_save = save_model_path + "ppi_p_rate_{}_edgeeps_{}_model_run_{}.pt".format(private_edge_rate,epsilon_feat, epsilon_edge, run+1)
    ...
    all_result["ppi_p_rate_{}_edgeeps_{}_model_run_{}".format(private_edge_rate,epsilon_feat, epsilon_edge, run+1)] = f1
    temp_f1.append(f1)
avg_result["ppi_p_rate_{}_edgeeps_{}".format(private_edge_rate,epsilon_feat, epsilon_edge)] = np.mean(np.array(temp_f1))
